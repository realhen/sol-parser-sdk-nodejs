...
__export(browser_exports, {
  ...
  bigintToJsonReplacer: () => bigintToJsonReplacer,
  defaultPubkey: () => defaultPubkey,
  dexEventToJsonString: () => dexEventToJsonString,
  elapsedMicrosSince: () => elapsedMicrosSince,
  ...
  programIds: () => program_ids_exports,
...
var program_ids_exports = {};
__export(program_ids_exports, {
  METEORA_DAMM_V2_PROGRAM_ID: () => METEORA_DAMM_V2_PROGRAM_ID,
  METEORA_DBC_PROGRAM_ID: () => METEORA_DBC_PROGRAM_ID,
  METEORA_DLMM_PROGRAM_ID: () => METEORA_DLMM_PROGRAM_ID,
  METEORA_POOLS_PROGRAM_ID: () => METEORA_POOLS_PROGRAM_ID,
  ORCA_WHIRLPOOL_PROGRAM_ID: () => ORCA_WHIRLPOOL_PROGRAM_ID,
  PUMPFUN_PROGRAM_ID: () => PUMPFUN_PROGRAM_ID,
  PUMPSWAP_FEES_PROGRAM_ID: () => PUMPSWAP_FEES_PROGRAM_ID,
  ...
  RAYDIUM_LAUNCHLAB_PROGRAM_ID: () => RAYDIUM_LAUNCHLAB_PROGRAM_ID
});
...
function hasDiscriminator(data, disc5) {
  if (data.length < disc5.length)
    return false;
  for (let i = 0; i < disc5.length; i++) {
    if (data[i] !== disc5[i])
      return false;
  }
  return true;
...
function bodyAfterDiscriminator(account, disc5, bodySize) {
  if (account.data.length < 8 + bodySize || !hasDiscriminator(account.data, disc5))
    return null;
...
var MAX_SHAREHOLDERS2 = 64;
var MAX_FEE_TIERS2 = 64;
function bnI643(v) {
  return v ?? 0n;
}
function bnU642(v) {
...
function eventTypeFromDexEvent2(ev) {
...
function applyActualEventTypeFilter(ev, filter) {
  if (!ev || !filter)
    return ev;
  const actual = eventTypeFromDexEvent2(ev);
...
function parseLogOptimized(log, signature, slot, txIndex, blockTimeUs, grpcRecvUs, eventTypeFilter, isCreatedBuy, recentBlockhash, programId) {
  if (programId === RAYDIUM_AMM_V4_PROGRAM_ID && log.indexOf("ray_log: ") >= 0) {
    if (eventTypeFilter && !eventTypeFilter.shouldInclude("RaydiumAmmV4Swap"))
      return null;
    const rb2 = recentBlockhash && recentBlockhash.length > 0 ? import_bs584.default.encode(recentBlockhash) : void 0;
  ...
  const rb = recentBlockhash && recentBlockhash.length > 0 ? import_bs584.default.encode(recentBlockhash) : void 0;
